...
public struct EventType: Equatable {
    ...
    public static let fingerDown = EventType(SDL_FINGERDOWN)
    public static let fingerMotion = EventType(SDL_FINGERMOTION)
    public static let fingerUp = EventType(SDL_FINGERUP)
    ...
    public static func from(event: SDL_Event) -> EventType {
        return EventType(SDL_EventType(event.type))
    }
