...
class ColorNode: RenderableNode {
    ...
    override func draw(renderer: CCRenderer, transform: GLKMatrix4) {
        let buffer = renderer.enqueueTriangles(2, andVertexes: 4, withState: renderState, globalSortOrder: 0)
        
        let w = Float(contentSizeInPoints.width)
        let h = Float(contentSizeInPoints.height)
        let zero = GLKVector2(v: (0.0, 0.0))
        
        let blueColor = CCColor.blueColor().glkVector4
        let redColor = CCColor.redColor().glkVector4
        let greenColor = CCColor.greenColor().glkVector4
