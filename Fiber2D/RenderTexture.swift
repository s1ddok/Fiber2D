...
class RenderTexture: RenderableNode {
    ...
    init(width w: Int, height h: Int) {
        super.init()
        self.contentScale = CCSetup.sharedSetup().assetScale
        self.contentSize = CGSizeMake(CGFloat(w), CGFloat(h))
        self.projection = GLKMatrix4MakeOrtho(0.0, Float(w), 0.0, Float(h), -1024.0, 1024.0)
        let rtSprite: RenderTextureSprite = RenderTextureSprite(texture: CCTexture.none(), rect: CGRectZero, rotated: false)
    ...
    func begin() -> CCRenderer {
        ...
        self.previousRenderer = CCRenderer.currentRenderer()
    ...
    func beginWithClear(r: Float, g: Float, b: Float, a: Float, flags: MTLLoadAction = .Clear) -> CCRenderer {
    ...
    private func FlipY(matrix: GLKMatrix4) -> GLKMatrix4 {
    ...
    func createTextureAndFboWithPixelSize(pixelSize: CGSize) {
    ...
    func create() {
        let size: CGSize = self.contentSize
        let pixelSize: CGSize = CGSizeMake(size.width * CGFloat(contentScale), size.height * CGFloat(contentScale))
    ...
    var contentScale: Float = CCSetup.sharedSetup().assetScale {
        didSet {
